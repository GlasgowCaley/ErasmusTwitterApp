...
public class Landpage extends Application{
	...
	@FXML private  	Button test;		// Used to reduce the window height .
	...
	public void start(final Stage landpage) throws Exception {
        ...
        mainVbox = (VBox) main.lookup("#mainVbox");
        	menuBar = (HBox) main.lookup("#menuBar");
        		...
        		btnFullScreen = (Button) main.lookup("#btnFullScreen");
        ...
        main.setOnKeyPressed(new EventHandler<KeyEvent>()
		{
		    ...
		    {
		    	/**
		    	 * This part of code (made by Alan) wants to help the full screen mode.Its intention is that when the ESC button is pressed the app doesn´t break
		    	 */
		        if (keyEvent.getCode().equals(KeyCode.ESCAPE))
		        {
					landpage.setFullScreen(false);//Quit full screen
					tweetView.setMinWidth(landpage.getWidth());
	...
	private void setBindings(){
	        btnRefold.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					...
					if(contentPaneDisplayed){
						...
						btnRefold.setText("^");//Changed.I thought that should be like the same text of the beggining(^) and not another (<)
	        ...
	        btnFullScreen.setOnAction(new EventHandler<ActionEvent>() {
	        	/**
		         * @author Juan Robisco
		         * @Function Button turns the window into the full screen
		         * @test Full screen mode has been tested in MAC OS and doesn´t work perfectly  
		         * @test It works perfectly on Windows OS
		         */
	        	@Override
				public void handle(ActionEvent event) {
					
					if(!landpage.isFullScreen()){
						landpage.setFullScreen(true);//Change into full screen
						tweetView.setMinWidth(landpage.getWidth());//Expands the tweets
						footer.setDisable(true); //It's not needed in full screen
						btnFullScreen.setText("↘");	//Changes the icon
					}
					else{
						landpage.setFullScreen(false);//Quit full screen
						tweetView.setMinWidth(landpage.getWidth());//Adapt the tweets to the window
						footer.setDisable(false); 
						btnFullScreen.setText("↔");//Changes the icon
					}
				}
